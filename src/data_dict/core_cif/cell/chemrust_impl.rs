...
pub(crate) fn basic_cell_data<T: UnitCellParameters>(value: &T) -> Vec<DataItems> {
    let cell_terms = [
        ...
        CellTerms::Angle_alpha(value.angle_alpha().to_degrees().into()),
        CellTerms::Angle_beta(value.angle_beta().to_degrees().into()),
        CellTerms::Angle_gamma(value.angle_gamma().to_degrees().into()),
    ];
